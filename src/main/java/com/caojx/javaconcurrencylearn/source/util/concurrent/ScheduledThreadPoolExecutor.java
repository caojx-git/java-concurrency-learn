...
/**
 * 源码分析参考文章：https://segmentfault.com/a/1190000016672638
 * <p>
 * ScheduledThreadPoolExecutor：
 * 在executors框架概述一节中，我们曾经提到过一种可对任务进行延迟/周期性调度的执行器（Executor），这类Executor一般实现了ScheduledExecutorService这个接口。
 ...
 * ScheduledThreadPoolExecutor其实是继承了ThreadPoolExecutor这个普通线程池，我们知道ThreadPoolExecutor中提交的任务都是实现了Runnable接口，
 * 但是ScheduledThreadPoolExecutor比较特殊，由于要满足任务的延迟/周期调度功能，它会对所有的Runnable任务都进行包装，包装成一个RunnableScheduledFuture任务
 * <p>
 ...
 * <p>
 * 另外，maximumPoolSize这个参数对ScheduledThreadPoolExecutor其实并没有作用，因为除非把corePoolSize设置为0，这种情况下ScheduledThreadPoolExecutor只会创建一个属于非核心线程池的工作线程；否则，
 * ScheduledThreadPoolExecutor只会新建归属于核心线程池的工作线程，一旦核心线程池满了，就不再新建工作线程。
 * <p>
...
public class ScheduledThreadPoolExecutor
     ...
     * <p>
     * ScheduledThreadPoolExecutor中的任务队列——DelayedWorkQueue，保存的元素就是ScheduledFutureTask。DelayedWorkQueue是一种堆结构，
     * time最小的任务会排在堆顶（表示最早过期），每次出队都是取堆顶元素，这样最快到期的任务就会被先执行。如果两个ScheduledFutureTask的time相同，
     * 就比较它们的序号——sequenceNumber，序号小的代表先被提交，所以就会先执行。
     * <p>
     * ScheduledFutureTask，ScheduledFutureTask是ScheduledThreadPoolExecutor这个线程池的默认调度任务类。
     * ScheduledFutureTask在普通{@link FutureTask}的基础上增加了周期执行/延迟执行的功能。它其实是通过继承FutureTask和Delayed接口来实现周期/延迟功能的
    ...
    private class ScheduledFutureTask<V>
        ...
        /**
         * ScheduledFutureTask的源码非常简单，基本都是委托FutureTask来实现的，关键是看下运行任务的方
         *
        ...
        public void run() {
            ...
            else if (ScheduledFutureTask.super.runAndReset()) { // 周期任务：调用FutureTask的runAndReset方法运行 ( FutureTask 的runAndReset方法与run方法的区别就是当任务正常执行完成后，不会设置任务的最终状态（即保持NEW状态），以便任务重复执行)
                setNextRunTime();
    ...
    public ScheduledThreadPoolExecutor(int corePoolSize) {
        // 内部其实都是调用了父类ThreadPoolExecutor的构造器，这里最需要注意的就是任务队列的选择——DelayedWorkQueue（DelayQueue的变种）
    ...
    public ScheduledFuture<?> schedule(Runnable command,
        ...
        // decorateTask方法把Runnable任务包装成ScheduledFutureTask
        // ScheduledFutureTask是RunnableScheduledFuture接口的实现类，任务通过period字段来表示任务类型
     ...
     * <p>
     * DelayedWorkQueue是一个无界队列，在队列元素满了以后会自动扩容，它并没有像DelayQueue那样，将队列操作委托给PriorityQueue，
     * 而是自己重新实现了一遍堆的核心操作——上浮、下沉。我这里不再赘述这些堆操作，读者可以参考PriorityBlockingQueue自行阅读源码
     * <p>
