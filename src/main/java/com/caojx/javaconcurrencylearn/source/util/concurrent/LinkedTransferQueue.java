...
/**
 * 源码分析参考文章：https://segmentfault.com/a/1190000016460411
 ...
 * <p>
 * LinkedTransferQueue可以说是LinkedBolckingQueue 和 SynchronousQueue 和合体
 *
 * <p>
...
public class LinkedTransferQueue<E> extends AbstractQueue<E>
    ...
    static final class Node {
        ...
        final boolean isData;   // false if this is a request node      // true: 数据结点; false: 请求结点

        // Node结点的值保存在item字段，匹配前后值会发生变化；
        volatile Object item;   // initially non-null if isData; CASed to match // 结点值
        volatile Node next;                                             // 后驱结点指针
        volatile Thread waiter; // null until waiting                   // 等待线程
     ...
     *
     * 执行类型，立即返回的NOW，异步的ASYNC，阻塞的SYNC， 带超时的 TIMED
     ...
     *
     * @param e        the item or null for take            入队操作, e非null; 出队操作, e为null
     * @param haveData true if this is a put, else a take   true表示入队元素, false表示出队元素
     * @param how      NOW, ASYNC, SYNC, or TIMED           NOW, ASYNC, SYNC, TIMED 四种常量定义(执行类型，立即返回的NOW，异步的ASYNC，阻塞的SYNC， 带超时的 TIMED)
     * @param nanos    timeout in nanosecs, used only if mode is TIMED  限时模式下使用(纳秒)
     * @return an item if matched, else e
     * @throws NullPointerException if haveData mode but e is null  匹配成功则返回匹配的元素, 否则返回e本身
     */
    private E xfer(E e, boolean haveData, int how, long nanos) {
        ...
        for (; ; ) {                            // restart on append race
            for (Node h = head, p = h; p != null; ) { // find & match first node        // 获取head结点
                boolean isData = p.isData;                                              // head结点数据类型
                Object item = p.item;                                                   // head结点值
                if (item != p && (item != null) == isData) { // unmatched               // 如果结点还未匹配过
                    if (isData == haveData)   // can't match                            // 同种类型结点不能匹配
                        break;
                    if (p.casItem(item, e)) { // match                                  // p指向从队首开始向后的第一个匹配结点
                        for (Node q = p; q != h; ) {
            ...
            if (how != NOW) {                 // No matches available
                if (s == null)
                    s = new Node(e, haveData);      // 创建一个入队结点
                Node pred = tryAppend(s, haveData); // 添加到队尾，pred指向s的前驱结点或s（队列中只有一个结点）或null（tryAppend失败）
                if (pred == null)
                    continue retry;           // lost race vs opposite mode // 入队失败，则重试
                if (how != ASYNC)
     ...
     *
     * @param s        the node to append                           待添加的结点
     * @param haveData true if appending in data mode               数据结点
     * @return null on failure due to losing race with append in    返回null表示失败; 否则返回s的前驱结点（没有前驱则返回s自身）
    ...
    private Node tryAppend(Node s, boolean haveData) {
        for (Node t = tail, p = t; ; ) {        // move p to last node and append
            ...
            else if ((n = p.next) != null)    // not last; keep traversing     // CASE3: 遍历至队尾结点
     ...
     * 3.阻塞线程，等待与之匹配的结点的到来。
     * <p>
     * 从awaitMatch方法其实可以看到一种经典的“锁优化”思路，就是 自旋 -> yield -> 阻塞，线程不会立即进入阻塞，因为线程上下文切换的开销往往比较大，
     * 所以会先自旋一定次数，中途可能伴随随机的yield操作，让出cpu时间片，如果自旋次数用完后，还是没有匹配线程出现，再真正阻塞线程。
     * <p>
    ...
    private E awaitMatch(Node s, Node pred, E e, boolean timed, long nanos) {
        ...
        int spins = -1; // initialized after first item and cancel checks   // 自旋次数, 锁优化操作
        ThreadLocalRandom randomYields = null; // bound if needed

        for (; ; ) {
            Object item = s.item;
            if (item != e) {                  // matched                    // CASE1: 匹配成功
                // assert item != s;
                s.forgetContents();           // avoid garbage              // 设置当前结点的值为自身，设置当前结点的等待线程为null.
                return LinkedTransferQueue.<E>cast(item);
            }
            if ((w.isInterrupted() || (timed && nanos <= 0)) &&
                    s.casItem(e, s)) {        // cancel                     // CASE2: 取消（线程被中断或超时）
                unsplice(pred, s);
                return e;
            }
            ...
            } else if (spins > 0) {             // spin                         // 自选次数减1
                --spins;
                if (randomYields.nextInt(CHAINED_SPINS) == 0)
                    Thread.yield();           // occasionally yield             // 随机yield线程
            } else if (s.waiter == null) {
                s.waiter = w;                 // request unpark then recheck    // waiter保存待阻塞线程
            } else if (timed) {                                                 // 限时等待情况, 计算剩余有效时间
                nanos = deadline - System.nanoTime();
                if (nanos > 0L)
                    LockSupport.parkNanos(this, nanos);
            } else {
                LockSupport.park(this);                                  // CASE4: 阻塞线程
            }
        }
    }
    ...
    public void transfer(E e) throws InterruptedException {
        if (xfer(e, true, SYNC, 0) != null) {
            // 进入到此处, 说明调用线程被中断了
            Thread.interrupted(); // failure possible only due to interrupt // 清除中断状态, 然后抛出中断异常
