...
/**
 * 源码分析参考文章：
 * https://segmentfault.com/a/1190000016315487
 * <p>
 ...
 * takeLock用于控制出队的并发，putLock用于入队的并发。这也就意味着，同一时刻，只能只有一个线程能执行入队/出队操作，其余入队/出队线程会被阻塞；
 * 但是，入队和出队之间可以并发执行，即同一时刻，可以同时有一个线程进行入队，另一个线程进行出队，这样就可以提升吞吐量。
 * <p>
 * 在ArrayBlockingQueue章节中，我们说过，ArrayBlockingQueue维护了一把全局锁，无论是出队还是入队，都共用这把锁，这就导致任一时间点只有一个线程能够执行。
 * 那么对于“生产者-消费者”模式来说，意味着生产者和消费者不能并发执行。
 * <p>
 * 总结：
...
//     }
    ...
    public LinkedBlockingQueue(Collection<? extends E> c) {
        this(Integer.MAX_VALUE);
        final ReentrantLock putLock = this.putLock;
        putLock.lock(); // Never contended, but necessary for visibility    // 这里加锁仅仅是为了保证可见性
        try {
            int n = 0;
            for (E e : c) {
                if (e == null)              // 队列不能包含null元素
                    throw new NullPointerException();
                if (n == capacity)          // 超过容量，队列已满
    ...
    public void put(E e) throws InterruptedException {
        ...
        try {
            ...
            c = count.getAndIncrement();            // 自增1，并返回自增前的元素个数，c表示入队前的队列元素个数
            if (c + 1 < capacity)                   // 入队后队列未满, 则唤醒一个“入队线程”
                notFull.signal();
        } finally {
            putLock.unlock();
        }
        if (c == 0)     // 如果队列为空了，说明何有可能很快就有入队元素，所以先唤醒一个出队线程，以提升性能
            signalNotEmpty();
    }
    ...
    public E take() throws InterruptedException {
        ...
        try {
            ...
            x = dequeue();
            c = count.getAndDecrement();                // 元素个数减1，并返回自减前的数，c表示出队前的元素个数
            if (c > 1)                                  // 出队前队列非空, 则唤醒一个出队线程
                notEmpty.signal();
        } finally {
            takeLock.unlock();
        }
        if (c == capacity)   // 队列满了，说明很快将会有出队线程从队列中获取元素，之后队列就可以重新入队了，所以先唤醒一个入队线程，以提升性能
            signalNotFull();
        return x;
    }
