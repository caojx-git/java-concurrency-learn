...
/**
 * 源码分析参考文章：https://segmentfault.com/a/1190000016214572
 * <p>
 * <p>
 ...
 * 大多数业务场景都是一种“读多写少”的情形，CopyOnWriteArrayList就是为适应这种场景而诞生的。CopyOnWriteArrayList，运用了一种“写时复制”的思想。
...
public class CopyOnWriteArrayList<E>
    ...
    // 查询——get方法
    // get方法并没有加锁，直接返回了内部数组对应索引位置的值：array[index]
    ...
    public E remove(int index) {
        ...
        try {
            Object[] elements = getArray();
            int len = elements.length;
            E oldValue = get(elements, index); // 获取index对应的元素, 用于返回
            int numMoved = len - index - 1;
            if (numMoved == 0)                 // index位置刚好是最后一个元素，直接删除最后一个元素
                setArray(Arrays.copyOf(elements, len - 1));
            else {                             // index位置不是最后一个元素，则删除index位置对应的元素
