...
import com.caojx.javaconcurrencylearn.source.util.concurrent.locks.AbstractQueuedSynchronizer;
 ...
 * 在juc-executors框架概述的章节中（https://segmentfault.com/a/1190000016586578），我们已经简要介绍过ThreadPoolExecutor了，通过Executors工厂，
 * 用户可以创建自己需要的执行器对象。ThreadPoolExecutor，它是J.U.C在JDK1.5时提供的一种实现了ExecutorService接口的执行器，或者说线程池。
 * ThreadPoolExecutor并没有自己直接实现ExecutorService接口，因为它只是其中一种Executor的实现而已，所以Doug Lea把一些通用部分封装成一个抽象父类——AbstractExecutorService，
 * 供J.U.C中的其它执行器继承。如果读者需要自己实现一个Executor，也可以继承该抽象类。
 * <p>
 * 线程池简介
 * 回到ThreadPoolExecutor，从该类的命名也可以看出，这是一种线程池执行器。线程池大家应该并不陌生，应用开发中经常需要用到数据库连接池，数据库连接池里维护着一些数据库连接，
 * 当应用需要连接数据库时，并不是自己创建连接，而是从连接池中获取可用连接；当关闭数据库连接时，只是将该连接还给连接池，以供复用。
 * <p>
 * 而线程池也是类似的概念，当有任务需要执行时，线程池会给该任务分配线程，如果当前没有可用线程，一般会将任务放进一个队列中，当有线程可用时，再从队列中取出任务并执行
 * 线程池的引入，主要解决以下问题：
 * 1.减少系统因为频繁创建和销毁线程所带来的开销；
 * 2.自动管理线程，对使用方透明，使其可以专注于任务的构建。
 * <p>
 * 总结：
 * 最后，我们来回顾下ThreadPoolExecutor的整体结构，ThreadPoolExecutor的核心方法是execute，控制着工作线程的创建和任务的执行，ThreadPoolExecutor中有几个比较重要的组件：阻塞队列、核心线程池、拒绝策略
 * 关于ThreadPoolExecutor这个线程池，最重要的是根据系统实际情况，合理进行线程池参数的设置以及阻塞队列的选择。现实情况下，一般会自己通过ThreadPoolExecutor的构造器去构建线程池，而非直接使用Executors工厂创建，
 * 因为这样更利于对参数的控制和调优。
 * 另外，根据任务的特点，要有选择的配置核心线程池的大小：
 * 1.如果任务是 CPU 密集型（需要进行大量计算、处理），则应该配置尽量少的线程，比如 CPU 个数 + 1，这样可以避免出现每个线程都需要使用很长时间但是有太多线程争抢资源的情况；
...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    /**
     * 既然是线程池，那么必然有线程池状态，同时也涉及对其中的工作线程（Worker）的管理，ThreadPoolExecutor是如何做的呢？
    ...
    private final class Worker
        ...
        Worker(Runnable firstTask) {
            setState(-1); // inhibit interrupts until runWorker // 初始的同步状态值
            // 这里把提交的任务保存到Worker里，在后面执行的时候，从Worker里取得这个任务执行
            this.firstTask = firstTask;
            // 通过线程工厂创建thread，在生成“内部线程”时，把Worker本身，当做参数传给了“内部线程”
     ...
     * 首先将Runnable任务包装成一个Worker对象，然后加入到一个工作线程集合中（名为workers的HashSet），最后调用工作线程中的Thread对象的start方法执行任务，其实最终是委托到Worker的下面run方法执行
     * <p>
     ...
     *                  prestartCoreThread or to replace other dying workers.   如果指定了该参数，表示将立即创建一个新的工作线程执行该firstTask任务；否则复用已有的工作线程，从工作队列中获取任务并执行
     * @param core      if true use corePoolSize as bound, else
     *                  maximumPoolSize. (A boolean indicator is used here rather than a
     *                  value to ensure reads of fresh values after checking other pool
     *                  state).                                                 执行任务的工作线程归属于那个线程池，true-核心线程池， false-非核心线程池
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        try {
            ...
            if (t != null) {
                ...
                if (workerAdded) {
                    t.start();              // 启动线程，其实最终是委托到Worker的下面run方法执行
                    workerStarted = true;
                }
            }
        } finally {
            if (!workerStarted)
                addWorkerFailed(w);     // 创建/启动工作线程失败, 需要执行回滚操作
        }
    ...
    /**
     * 任务的获取，也就是runWorker中使用的getTask方法
    ...
    private Runnable getTask() {
        ...
        for (; ; ) {
            ...
            try {
                Runnable r = timed ? workQueue.poll(keepAliveTime, TimeUnit.NANOSECONDS) : workQueue.take();  // 从队列中获取任务
                if (r != null)
                    return r;
    ...
    final void runWorker(Worker w) {
        ...
        try {
            ...
            // 当task==null时会通过getTask从队列取任务，getTask方法用于从任务队列中获取一个任务，如果获取不到任务，会跳出while循环，最终会通过processWorkerExit方法清理工作线程。
            ...
            // 1.直接执行到此处, 说明该工作线程自身既没有携带任务, 也没从任务队列中获取到任务
            // 2.while 完成，任务正常执行完成
    ...
    /**
     * Executors工厂方法创建的三种线程池：newFixedThreadPool、newSingleThreadExecutor、newCachedThreadPool，内部都是通过ThreadPoolExecutor的下面这个构造器实例化了ThreadPoolExecutor对象
     ...
     *
     * @param corePoolSize    the number of threads to keep in the pool, even
     *                        if they are idle, unless {@code allowCoreThreadTimeOut} is set    核心线程池中的最大线程数
     * @param maximumPoolSize the maximum number of threads to allow in the
     *                        pool                                                              总线程池中的最大线程数
     * @param keepAliveTime   when the number of threads is greater than
     *                        the core, this is the maximum time that excess idle threads
     *                        will wait for new tasks before terminating.                       空闲线程的存活时间
     * @param unit            the time unit for the {@code keepAliveTime} argument              keepAliveTime的单位
     * @param workQueue       the queue to use for holding tasks before they are
     *                        executed.  This queue will hold only the {@code Runnable}
     *                        tasks submitted by the {@code execute} method.                    任务队列, 保存已经提交但尚未被执行的线程
     * @param threadFactory   the factory to use when the executor
     *                        creates a new thread                                              线程工厂(用于指定如果创建一个线程)
     * @param handler         the handler to use when execution is blocked
     *                        because the thread bounds and queue capacities are reached        拒绝策略 (当任务太多导致工作队列满时的处理策略)
     * @throws IllegalArgumentException if one of the following holds:<br>
     *                                  {@code corePoolSize < 0}<br>
     *                                  {@code keepAliveTime < 0}<br>
     *                                  {@code maximumPoolSize <= 0}<br>
     ...
     * 2.如果工作线程数大于等于CorePoolSize，则尝试将任务加入到队列等待以后执行。如果加入队列失败了（比如队列已满的情况），
    ...
    public void execute(Runnable command) {
        ...
        // 执行到此处，说明工作线程创建失败 或 工作线程数 >= 核心线程上限
        if (isRunning(c) && workQueue.offer(command)) { // CASE2: 插入任务至队列
            ...
            if (!isRunning(recheck) && remove(command))
                ...
                // 一般来讲每个工作线程（Worker）都有一个Runnable任务和一个对应的执行线程Thread，当我们调用addWorker方法时，如果不传入相应的任务，
                // 那么就只是新建了一个没有任务的工作线程（Worker），该Worker就会从工作队列中取任务来执行（因为自己没有绑定任务）。如果传入了任务，新建的工作线程就会执行该任务
                addWorker(null, false);

        } else if (!addWorker(command, false))   // CASE3: 插入队列失败，判断工作线程数 < 总线程池上限的情况下，立即添加一个工作线程执行任务
    ...
    /**
     * 添加工作线程
     * ensurePrestart可以看到，如果核心线程池未满，则新建的工作线程会被放到核心线程池中。如果核心线程池已经满了，
    ...
    /**
     * 拒绝策略-AbortPolicy策略其实就是抛出一个RejectedExecutionException异常
     * <p>
    ...
    /**
     * ThreadPoolExecutor一共提供了4种拒绝策略-DiscardPolicy策略其实就是无为而治，什么都不做，等任务自己被回收：
     * <p>
    ...
    /**
     * 拒绝策略-DiscardOldestPolicy策略是丢弃任务队列中的最近一个任务，并执行当前任务：
     * <p>
