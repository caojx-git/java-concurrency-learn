...
/**
 * 源码分析参考文章：https://segmentfault.com/a/1190000016398508
 * LinkedBlockingDeque 和 ConcurrentLinkedDeque类似，都是一种双端队列的结构，只不过LinkedBlockingDeque同时也是一种阻塞队列，它是在JDK1.6时随着J.U.C包引入的，实现了BlockingDueue接口，底层基于双链表实现。
 ...
 * <p>
 * 总结
 * LinkedBlockingDeque作为一种阻塞双端队列，提供了队尾删除元素和队首插入元素的阻塞方法。该类在构造时一般需要指定容量，如果不指定，则最大容量为Integer.MAX_VALUE。
 ...
 * 考虑下面这样一种场景：有多个消费者，每个消费者有自己的一个消息队列，生产者不断的生产数据扔到队列中，消费者消费数据有快又慢。
 * 为了提升效率，速度快的消费者可以从其它消费者队列的队尾出队元素放到自己的消息队列中，由于是从其它队列的队尾出队，这样可以减少并发冲突（其它消费者从队首出队元素），
...
public class LinkedBlockingDeque<E>
    ...
    /**
     * 全局锁
     * LinkedBlockingDeque利用ReentrantLock来保证线程安全，所有对队列的修改操作都需要先获取这把全局锁
     * <p>
    ...
    private boolean linkFirst(Node<E> node) {
        ...
        if (count >= capacity)  // 队列已满，直接返回false
            return false;
     ...
     * 如果队列已满, 则阻塞线程.
     * <p>
     * put方法内部调用了putLast方法，这是Deque接口独有的方法。入队操作的关键是linkLast方法
     *
     * @throws NullPointerException {@inheritDoc}
     * @throws InterruptedException {@inheritDoc}
     */
