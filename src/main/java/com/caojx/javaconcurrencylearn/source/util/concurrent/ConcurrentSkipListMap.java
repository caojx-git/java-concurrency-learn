...
/**
 * 源码分析参考文章：https://segmentfault.com/a/1190000016168566
 * <p>
...
public class ConcurrentSkipListMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * initialize方法将一些字段置初始化null，然后将head指针指向新创建的HeadIndex结点
     * <p>
    ...
    static final class Node<K, V> {
        ...
        void helpDelete(Node<K, V> b, Node<K, V> f) {
             ...
             *
             * 重新检查一遍结点位置，确保b和f分别为当前结点的前驱/后继
             */
            if (f == next && this == b.next) {
                if (f == null || f.value != f) // not already marked f为null或非标记结点
        ...
        SimpleImmutableEntry<K, V> createSnapshot() {
            Object v = value;
            if (v == null || v == this || v == BASE_HEADER) // 标记结点或最底层头结点，直接返回null
                return null;
            @SuppressWarnings("unchecked") V vv = (V) v;
    ...
    /**
     * findPredecessor用于查找“小于且最接近给定key”的Node结点，并且这个Node结点必须有上层结点，如果不存在这样的数据结点，则返回底层链表的头结点.
     * <p>
     ...
     * @param onlyIfAbsent if should not insert if already present  onlyIfAbsent true: 仅当Key不存在时才进行插入
     * @return the old value, or null if newly inserted
     */
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        Node<K, V> z;             // added node z指向待添加的Node结点
        ...
        for (; ; ) {
            ...
            for (Node<K, V> b = findPredecessor(key, cmp), n = b.next; ; ) {
                if (n != null) {                    // b存在后驱结点：  b -> n -> f
                    ...
                    if (n != b.next)               // inconsistent read  存在并发修改,放弃并重试
                        break;
                    if ((v = n.value) == null) {   // n is deleted  n为标记删除结点
                        n.helpDelete(b, f);
                        break;
                    }
                    if (b.value == null || v == n) // b is deleted  b为标记删除结点
                        break;
                    ...
                    if (c == 0) {                       // 存在Key相同的结点
                        ...
                        break; // restart if lost race to replace value // CAS更新失败,则重试
                    }
                ...
                if (!b.casNext(n, z))   // 尝试插入z结点： b -> z -> n
                    break;         // restart if lost race to append to b   CAS插入失败，则重试
                break outer;    // 跳出最外层循环
            }
        }
        ...
        for (; ; ) {
            ...
            for (Node<K, V> b = findPredecessor(key, cmp), n = b.next; ; ) {    // b -> n
                ...
                if (n == null)
                    break outer;
                Node<K, V> f = n.next;              // b -> n -> f
                if (n != b.next)                    // inconsistent read    一致性判断
                    break;
                if ((v = n.value) == null) {        // n is deleted         n is deleted
                    n.helpDelete(b, f);
                    break;
                }
                if (b.value == null || v == n)      // b is deleted         b is deleted
                    break;
                if ((c = cpr(cmp, key, n.key)) < 0)
                    break outer;
                if (c > 0) {
                ...
                if (!n.appendMarker(f) || !b.casNext(n, f)) // n -> marker -> f
                    ...
                    findPredecessor(key, cmp);      // clean index  删除Index结点
    ...
    /**
     * 插入键值对，ConcurrentSkipListMap在插入键值对时，Key和Value都不能为null
     * <p>
     ...
     * {@code null} if there was no mapping for the key  如果key存在，返回旧value值；否则返回null
     * @throws ClassCastException   if the specified key cannot be compared
     *                              with the keys currently in the map
